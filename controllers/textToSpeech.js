  ...
  fetchIamToken().then((token) => {
    superagent
      ...
      .then((result) => {
        ...
        const outBuf = new Int16Array(result.body.length / 2);
        try {
          let count = 0;
          ...
          const ss = new Int16Array(START_PCM.length + outBuf.length + END_PCM.length);
          ss.set(START_PCM);
          ss.set(outBuf, START_PCM.length);
          ss.set(END_PCM, START_PCM.length + outBuf.length);
